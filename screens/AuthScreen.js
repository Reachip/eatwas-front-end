...
export default class AuthScreen extends Component {
  constructor(props) {
    ...
    AsyncStorage.getItem('token').then(token => {
      if (token !== null) {
        jwt
          .decode(token, 'ww4n708000!', {skipValidation: false})
          .then(decodedToken => {
                ...
                this.session
                  ...
                  .then(response => {
                    if (response.code === 401) {
                      ...
                      AsyncStorage.setItem('token', response.message).then(
                        () => {
                          navigate('Home');
                        },
                      );
                  ...
                  .catch(() =>
                    Alert.alert('Erreur', 'Une erreur interne a été détecté'),
                  );
              }}>
              Me connecter
            </Button>
